...
# Edited Dec 10, 2013 to correct the lower case to upper case.
# Edited Dec 30, 2015 to iterate through entirely
		...
		for toFind in sites:
			...
			for gRNA in myIter:
				if 'N' not in str(gRNA.group(0)) and str(gRNA.group(0)) not in gRNASeen:
					summaryfile.write('>' + str(seqID) + '_Plus_' + str(gRNA.start() + index) + '\n' + str(gRNA.group(0)) + '\n')
					count = count + 1
		...
		for toFindRC in sitesRC:
			...
			for gRNA in myIterRC:
				if 'N' not in str(gRNA.group(0)) and str(gRNA.group(0)) not in gRNASeen:
					...
					summaryfile.write('>' + str(seqID) + '_Minus_' + str(gRNA.start() + index) + '\n' + str(seqRC) + '\n')
					count = count + 1
					totalgRNAsFound = totalgRNAsFound + 1
					gRNASeen[str(gRNA.group(0))] = 'Y'
		index += 1
